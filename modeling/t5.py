...
from utils.layers import GatedCrossAttentionModule
...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        self.cross_gnp = config.cross_gnp
        self.xattn_after = config.xattn_after
        ...
        if self.is_decoder:
            ...
            #modified
            if config.cross_gnp:
                self.fusion = GatedCrossAttentionModule(config.d_model, config.dim_graph, config.num_graph, config.xattn_heads, config.xattn_mult)
            #---------------------
    ...
    def forward(
        ...
        #modified
        kv = None
        if self.is_decoder and self.cross_gnp and not self.xattn_after:
            if nodes_mask is None and nodes is not None:
                # media is of shape (batch_size, n_latents, d_media)
                batch_size, n_graphs, n_nodes = nodes.shape[:3]
                nodes_mask = torch.ones(size=(batch_size, n_graphs, n_nodes), dtype=torch.long, device=nodes.device)
            if nodes is not None:
                hidden_states, kv = self.fusion(hidden_states, nodes, nodes_mask, previous_kv, use_cache)
        #---------------------
        ...
        if do_cross_attention:
            ...
            #modified
            kv = None
            if self.is_decoder and self.cross_gnp and self.xattn_after:
                if nodes_mask is None and nodes is not None:
                    # media is of shape (batch_size, n_latents, d_media)
                    batch_size, n_graphs, n_nodes = nodes.shape[:3]
                    nodes_mask = torch.ones(size=(batch_size, n_graphs, n_nodes), dtype=torch.long, device=nodes.device)
        ...
        if use_cache:
            if self.is_decoder and self.cross_gnp:
                outputs = outputs + (present_key_value_state,) + attention_outputs + (kv,)
            else:
...
class T5Stack(InvertibleAdaptersMixin, T5PreTrainedModel):
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if use_cache and self.is_decoder and self.cross_gnp:
                previous_kv = layer_outputs[-1]
...
class T5ForConditionalGeneration(T5ModelWithHeadsAdaptersMixin, T5ModelAdaptersMixin, T5PreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        nodes: Optional[torch.FloatTensor] = None, #graph data
        nodes_mask: Optional[torch.FloatTensor] = None #graph data
