...
def package_payload(argdict):
    ...
    params = []
    params.append(('inputtext', argdict['inputtext']))
...
class METAMAPOnline():
    def __init__(self) -> None:
        ...
        self.resultformat = 'mmi'
        self.sources = 'all'
        self.semantic_types = 'all'
        self.acceptfmt = 'text/plain'
    ...
    def get_entities(self, query):
        ...
        d['inputtext'] = query
        d['docformat'] = self.docformat
        d['resultformat'] = self.resultformat
        d['sources'] = self.sources
        d['semantic_types'] = self.semantic_types

        payload = package_payload(d)
        resp = handle_request(self.url, self.acceptfmt, payload)

        ss = resp.text.splitlines()
        entities = []
        for s in ss:
            parts = s.strip().split("|")
            if not parts or all(element == '' for element in parts):
                continue
            if len(parts) < 5:
                continue
            flag = 0
            exclude_sts=['qlco', 'qnco', 'tmco', 'ftcn']
            for ex in exclude_sts:
                if ex in parts[5]:
                    flag = 1

            if flag == 0:
                entities.append(parts[4])
...
def get_entities(query):
    ...
    d['inputtext'] = query
    d['docformat'] = 'freetext'
    d['resultformat'] = 'mmi'
    d['sources'] = 'all'
    d['semantic_types'] = 'all'

    payload = package_payload(d)
    acceptfmt = 'text/plain'
    url = 'https://ii.nlm.nih.gov/metamaplite/rest/annotate'
    resp = handle_request(url, acceptfmt, payload)
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        parser.add_argument('--inputtext', help='INPUT')
